...
async function convertFile(req, res, file) {
	if (file.downloaded) {
		try {
			...
			if (vcodec.trim() == "h264") {
				exec("ffmpeg -i \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + "\" -c copy -tune zerolatency -start_number 0 -hls_time 10 -hls_list_size 0 -f hls \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".m3u8\" && touch \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".done\"");
			} else {
				...
				if (parseInt(height.trim()) > 2160) {
					exec("ffmpeg -threads " + parseInt(Math.floor(os.cpus().length * 0.5)) + " -i \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + "\" -c:v libx264 -vf \"scale=-2:1080:flags=fast_bilinear\" -c:a copy -tune zerolatency -start_number 0 -hls_time 10 -hls_list_size 0 -f hls \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".m3u8\" && touch \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".done\"");
				} else {
