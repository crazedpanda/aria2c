		...
		if (vcodec.trim() == "h264") {
			...
			exec("ffprobe -v error -select_streams v:0 -show_entries stream=height -of default=noprint_wrappers=1:nokey=1 \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + "\"").then(function(height) {
				if (parseInt(height.trim()) > 2160) {
					var ffmpeg = child.spawn("ffmpeg", ["-i", "pipe:0", "-threads", parseInt(Math.floor(os.cpus().length * 0.5)), "-c:v", "libx264", "-profile:v", "baseline", "-vf", "scale=-2:1080:flags=lanczos", "-c:a", "copy", "-movflags", "+faststart", "-tune", "zerolatency", "-start_number", 0, "-hls_time", 10, "-hls_list_size", 0, "-f", "hls", "/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".m3u8"], null);
					...
					ffmpeg.stderr.pipe(fs.createWriteStream("/dev/stderr", {
						flags: "a"
					}));
