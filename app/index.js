...
async function convertFile(req, res, file) {
	const { stdout, stderr } = await exec("ffmpeg -i \"/tmp/webtorrent/" + file.path + "\" -c copy -start_number 0 -hls_time 10 -hls_list_size 0 -f hls \"/tmp/webtorrent/" + file.path + ".m3u8\"");
	if (stderr) {
		res.send(stderr);
	}
