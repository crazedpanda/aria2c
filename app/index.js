		...
		if (vcodec.trim() == "h264") {
			...
			exec("ffprobe -v error -select_streams v:0 -show_entries stream=height -of default=noprint_wrappers=1:nokey=1 \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + "\"").then(function(height) {
				if (parseInt(height.trim()) > 2160) {
					return exec("ffmpeg -i \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + "\" -c:v mpeg2video -qscale:v 2 -vf \"scale=-2:1080:flags=lanczos\" -c:a copy \"/tmp/webtorrent/" + req.params.infoHash + ".mkv\" && ffmpeg -i \"/tmp/webtorrent/" + req.params.infoHash + ".mkv\" -c:v libx264 -c:a copy -start_number 0 -hls_time 10 -hls_list_size 0 -f hls \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".m3u8\" && touch \"/tmp/webtorrent/" + req.params.infoHash + "/" + file.path + ".done\"", req.query.limit && req.query.limit.length ? parseInt(req.query.limit) : 0);
				} else {
